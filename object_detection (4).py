...
class ObjectDetector:
    ...
    def _run_inference(self, frame_rgb: np.ndarray) -> list:
        ...
        # Build a name → index map once so we can fetch by exact tensor name.
        # This bypasses the XNNPACK runtime-fusion issue where get_tensor() by
        # sorted index only returns 2 tensors after invoke() even though
        # get_output_details() reports 4.
        name_to_index = {d["name"]: d["index"] for d in self._output_details}

        try:
            boxes   = self.interpreter.get_tensor(
                name_to_index["TFLite_Detection_PostProcess"])[0]    # (N, 4)
            classes = self.interpreter.get_tensor(
                name_to_index["TFLite_Detection_PostProcess:1"])[0]  # (N,)
            scores  = self.interpreter.get_tensor(
                name_to_index["TFLite_Detection_PostProcess:2"])[0]  # (N,)
            count   = int(self.interpreter.get_tensor(
                name_to_index["TFLite_Detection_PostProcess:3"])[0]) # scalar
        except KeyError as e:
            print(f"[ObjectDetector] Warning: output tensor {e} not found. "
